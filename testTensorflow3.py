...
def add_layer(inputs,in_size,out_size,activation_function=None):
    with tfm.name_scope('layer'):
        with tfm.name_scope('weights'):
            Weights = tfm.Variable(tfm.random_normal([in_size,out_size]))
        with tfm.name_scope('biases'):
            biases = tfm.Variable(tfm.zeros([1,out_size]) + 0.1)
        with tfm.name_scope('Wx_plus_b'):
            Wx_plus_b = tfm.add(tfm.matmul(inputs,Weights), biases)
...
with tfm.name_scope('inputs'):
    xs = tfm.placeholder(tfm.float32,[None,1],name='x_in')
    ys = tfm.placeholder(tfm.float32,[None,1],name='y_in')
# Add a hide layer
...
# writer logs
writer = tfm.summary.FileWriter("logs/",session.graph)
